...
class CentralReportes extends CI_Controller {
  ...
  public function report_ficha_atencion()
  {
    ...
    foreach ($arrDetalleAtencion as $key => $fDetAtencion) {
      ...
      if($fDetAtencion['tipoProductoId'] == 1){
        ...
        $this->pdf->SetFont('Arial','B',9);
        $this->pdf->SetFillColor(214,225,242);
        $this->pdf->Cell(0,6,utf8_decode('MOTIVO DE CONSULTA'),1,0,'C', true);
        $this->pdf->Ln(8);
        $this->pdf->SetFont('Arial','',8);
        $this->pdf->MultiCell(0,6,utf8_decode($fDetAtencion['motivoConsulta']));
        // $this->pdf->Ln(4);
        // $this->pdf->SetFont('Arial','B',9);
        // $this->pdf->SetFillColor(214,225,242);
        ...
        // $this->pdf->Ln(4);
        // $this->pdf->SetFont('Arial','',8);
        // $this->pdf->Cell(47,6,$fAtencion['presionArterial']);
        ...
        $this->pdf->SetFont('Arial','B',9);
        $this->pdf->SetFillColor(214,225,242);
        $this->pdf->Cell(0,6,utf8_decode('ANTROPOMETRÍA Y SIGNOS VITALES'),1,0,'C', true);
        $this->pdf->Ln(8);
        $this->pdf->SetFont('Arial','B',9);
  ...
  public function reporte_produccion_medico()
  {
    ...
    $this->pdf->SetWidths(array(12, 22, 70, 20, 28, 78, 30, 25));
  ...
  public function reporte_produccion_general()
  {
    ...
    if($allInputs['tipoReporte']['id'] === 'RPMP'){
      $headerDetalle = array('N°', 'MÉTODO DE PAGO', 'CANTIDAD', 'MONTO');
      $this->pdf->SetAligns(array('L', 'C', 'R', 'R'));
      $this->pdf->SetWidths(array(5, 90, 30, 40));
    }
    ...
    if($allInputs['tipoReporte']['id'] === 'RPMP'){
      $lista = $this->model_cita->m_obtener_produccion_general_group_metodo_pago($allInputs);
      $this->pdf->SetFont('Arial','',8);
      foreach ($lista as $key => $row) {
        $this->pdf->Row( 
          array( 
    ...
    if($allInputs['tipoReporte']['id'] === 'RPP' || $allInputs['tipoReporte']['id'] === 'RPM' || $allInputs['tipoReporte']['id'] === 'RPMP'){
      $this->pdf->SetWidths(array(125, 40));
      $this->pdf->SetFont('Arial','B',12);
      $this->pdf->SetFillColor(224,235,255);
      $this->pdf->SetAligns(array('R', 'R'));

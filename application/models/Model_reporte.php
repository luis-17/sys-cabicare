...
class Model_reporte extends CI_Model {
	...
	public function m_cargar_activos($paramDatos){
		...
		$this->db->where('pg.fechaRegistro BETWEEN '. $this->db->escape( darFormatoYMD($paramDatos['desde']).' 00:00') .' AND ' 
			. $this->db->escape( darFormatoYMD($paramDatos['hasta']).' 11:59'));

		return $this->db->get()->result_array();
	}

    public function m_cargar_pasivos(){
        $this->db->select("do.numSerie, do.numDoc, do.mes, do.anio, do.dia, do.monto", FALSE);
        $this->db->select("CONCAT_WS('-', do.anio, do.mes, do.dia) AS fecha", FALSE);
		$this->db->from('documento do');
        $this->db->where_in('do.estado', array(1));
		$this->db->where("CONCAT_WS('-', do.anio, do.mes, do.dia) BETWEEN ". $this->db->escape( darFormatoYMD($paramDatos['desde']).' 00:00') .' AND ' 
			. $this->db->escape( darFormatoYMD($paramDatos['hasta']).' 11:59'));
